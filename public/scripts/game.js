...
setInterval(() => {
  for (var i = 1; i <= 15; i++) {
    if (i > gameState.chickenLane + 1) {
      Math.random() >= gameState.carFrequency &&
        genCar(i, Math.floor(Math.random() * 8) + 1);
    }
  }
...
function moveChickenLane() {
  const pipe = document.querySelector(
    `[lane="${gameState.chickenLane}"] [game-class="pipe"]`,
  );
  if (!pipe) return;
  ...
  } else if (gameState.chickenLane === 1)
...
function setDead(dead) {
  const lane = document.querySelector(
    `div.item[game-class="lane"][lane="${gameState.chickenLane}"]`,
  );
